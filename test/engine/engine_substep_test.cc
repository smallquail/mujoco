...
// Tests for engine/engine_substep.c.

#include "src/engine/engine_substep.h"
...
TEST_F(SubstepTest, SegSegDistance) {
  mjtNum seg1[6] = {-1, 0, 0, 1, 0, 0};
  mjtNum seg2[6] = {0, -1, 0.3, 0, 1, 0.3};
  mjtNum witness[6];
  mjtNum dist = mj_substepElemDistance(seg1, 1, seg2, 1, witness, nullptr);
...
TEST_F(SubstepTest, DistanceGradient) {
  ...
  for (int i = 0; i < 15; i++) {
    mjtNum save = all[i];
    all[i] = save + eps;
    mjtNum dp = mj_substepElemDistance(all, 1, all + 6, 2, nullptr, nullptr);
    all[i] = save - eps;
    mjtNum dm = mj_substepElemDistance(all, 1, all + 6, 2, nullptr, nullptr);
    all[i] = save;
...
TEST_F(SubstepTest, Broadphase) {
  ...
  n = mj_substepBroadphase(model, data, nullptr, nullptr, 0.05, pairs, 256);
...
TEST_F(SubstepTest, Feasibility) {
  mjModel* model = LoadModel(kTwoStringsXml);
  mjData* data = mj_makeData(model);
  mj_forward(model, data);
  EXPECT_EQ(mj_substepFeasible(model, data, nullptr), 1);
  ...
  mj_forward(model, data);
  mjSubstepPair bad;
  EXPECT_EQ(mj_substepFeasible(model, data, &bad), 0);
...
TEST_F(SubstepTest, SolveBlocksCrossing) {
  for (int enabled = 0; enabled < 2; enabled++) {
    mjModel* model = LoadModel(kCrossingStringsXml);
    if (!enabled) {
      model->opt.disableflags |= mjDSBL_SUBSTEP;
    }
    mjData* data = mj_makeData(model);
    ...
    for (int i = 0; i < 30; i++) {
      ...
      if (enabled) {
        EXPECT_EQ(mj_substepFeasible(model, data, nullptr), 1) << "step " << i;
      }
    }
    if (enabled) {
...
TEST_F(SubstepTest, SolveMatchesIntegration) {
...
TEST_F(SubstepTest, SolveResting) {
...
TEST_F(SubstepTest, SolveRestingGap) {
  mjModel* model = LoadModel(kRestingStringsXml);
  mjData* data = mj_makeData(model);
  int f1 = mj_name2id(model, mjOBJ_FLEX, "string1");
  int f2 = mj_name2id(model, mjOBJ_FLEX, "string2");
...
TEST_F(SubstepTest, SolveKeepsTangentialMomentum) {
...
TEST_F(SubstepTest, SolveElasticStable) {
...
TEST_F(SubstepTest, InfeasibleStartHandoff) {
  mjModel* model = LoadModel(kTwoStringsXml);
  mjData* data = mj_makeData(model);
  ...
  mj_forward(model, data);
  ASSERT_EQ(mj_substepFeasible(model, data, nullptr), 0);
