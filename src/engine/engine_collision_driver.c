...
void mj_collideElems(const mjModel* m, mjData* d, int f1, int e1, int f2, int e2) {
  ...
  // variational flex solver handoff: feasible pairs are owned by the barrier solve and
  ...
  if (!mjDISABLED(mjDSBL_FLEXIPC) && mj_ipcPairEligible(m, f1, f2)) {
    ...
    if (mj_ipcElemDistance(vert1, dim1, vert2, dim2, NULL, NULL) >
        mj_ipcLegacyDistance(m, f1, f2)) {
      return;
    }
  }
